...
const withChildren = (task: TaskNode, children: TaskNode[]): TaskNode => ({
  ...task,
  children,
  attachments: task.attachments ?? []
});

export const findTask = (tasks: TaskNode[], id: string): TaskNode | undefined => {
...
export const updateTask = (
  ...
  (tasks || []).map((task) =>
    task.id === id ? updater(task) : withChildren(task, updateTask(task.children || [], id, updater))
  );

export const addChild = (tasks: TaskNode[], parentId: string | null, child: TaskNode): TaskNode[] => {
  if (!parentId) {
    return [...(tasks || []), child];
  }
...
export const replaceChildren = (tasks: TaskNode[], parentId: string, newChildren: TaskNode[]): TaskNode[] =>
  (tasks || []).map((task) =>
    task.id === parentId
...
export const removeTask = (tasks: TaskNode[], id: string): TaskNode[] =>
  (tasks || [])
...
export const reorderWithinParent = (
  ...
  const move = (list: TaskNode[]): TaskNode[] => {
    const idx = list.findIndex((t) => t.id === targetId);
    if (idx === -1) return list.map((t) => withChildren(t, move(t.children || [])));
